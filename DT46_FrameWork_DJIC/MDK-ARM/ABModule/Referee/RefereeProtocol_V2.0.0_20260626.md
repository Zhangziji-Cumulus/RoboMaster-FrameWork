...
KeyboardMouseControl 与CustomControl)，
...
首次发布 
前言 
本通信协议在RMUC 与RMUL 两项赛事中的适用范围如下： 
 
对于在RMUC 与RMUL 之间共用的兵种、机制或场地道具等条目，本协议内容对两项赛事均适用； 
 
...
1 串口协议 
1.1 串口协议格式 
通信方式为串口的部分，性质如下：8 位数据位，1 位停止位，无硬件流控，无校验位。详细信息见以下表
格： 
表 1-1 波特率 
物理接口 
波特率 
...
 
常规链路由裁判系统服务器和主控模块进行数据转发，从电源管理模块的User 串口收发数据，示意
图如下： 
...
5：比赛结算中 
当前阶段剩余时间，单位：秒 
UNIX 时间，当机器人正确连接到裁判系统的NTP 服务器后生效 
...
大小 
说明 
己方1 号英雄机器人血量，若该机器人未上场或者被罚下，则血量为0，下文同理 
己方2 号工程机器人血量 
己方3 号步兵机器人血量 
己方4 号步兵机器人血量 
己方全队总伤害与对方全队总伤害之差 
...
bit 1：保留位 
 
bit 2：己方补给区的占领状态，1 为已占领（仅RMUL 适用） 
 
bit 3-6：己方能量机关状态 
 
bit 3-4：己方小能量机关的激活状态，0 为未激活，1 为已激活，
2 为正在激活 
 
bit 5-6：己方大能量机关的激活状态，0 为未激活，1 为已激活，
2 为正在激活 
 
...
开局默认为0） 
 
bit 20-22：对方飞镖最后一次击中己方前哨站或基地的具体目标，开
局默认为0，1 为击中前哨站，2 为击中基地固定目标，3 为击中基地
随机固定目标，4 为击中基地随机移动目标，5 为击中基地末端移动目
标 
 
bit 23-24：中心增益点的占领状态，0 为未被占领，1 为被己方占领，
2 为被对方占领，3 为被双方占领。（仅RMUL 适用） 
 
...
4：判负 
 
己方最后一次受到判罚的违规机器人ID（如红1 机器人ID 为1，蓝1 机
器人ID 为101） 
 
判负和双方黄牌时，该值为0 
...
电源管理模块的输出情况： 
 
bit 0：gimbal 口输出，0 为无输出，1 为24V 输出 
 
...
大小 
说明 
bit 0-3：当扣血原因为装甲模块被弹丸攻击、受撞击或离线时，该4 bit
组成的数值为装甲模块或测速模块的ID 编号；当其他原因导致扣血时，
该数值为0 
bit 4-7：血量变化类型 
 
0：装甲模块被弹丸攻击导致扣血 
...
0x0206 的受伤害情况为机器人裁判系统本地判定，即时发送，但实际是否受到对应伤害受规则
条例影响，请以服务器最终判定为准。 
表 1-17 0x0207 
字节偏移量 
大小 
...
大小 
说明 
机器人自身拥有的17mm 弹丸允许发弹量 
42mm 弹丸允许发弹量 
剩余金币数量 
堡垒增益点提供的储备17mm 弹丸允许发弹量；该值与机器人是否实际
占领堡垒无关 
typedef _packed struct 
{ 
...
注：所有RFID 卡仅在赛内生效。在赛外，即使检测到对应的RFID 卡，对应值也为0。 
typedef _packed struct 
{ 
...
场地围挡在红方补给站附近的交点为坐标原点，沿场地长边向蓝方为X 轴正方向，沿场地短边
向红方停机坪为Y 轴正方向。 
...
bit 5：对方哨兵机器人易伤情况 
 
bit 6：己方1 号英雄机器人特殊标识情况 
 
bit 7：己方2 号工程机器人特殊标识情况 
 
bit 8：己方3 号步兵机器人特殊标识情况 
 
bit 9：己方4 号步兵机器人特殊标识情况 
 
bit 10：己方空中机器人特殊标识情况 
 
bit 11：己方哨兵机器人特殊标识情况 
 
bit 12：对方空中机器人是否被己方雷达激
光瞄准 
 
bit 13：对方空中机器人当前是否处于被反
制状态 
 
bit 14：己方空中机器人是否被对方雷达激
光瞄准 
 
bit 15：己方空中机器人是否处于被反制状
态 
人被标记进度≥100 时发
送1，被标记进度<100
时发送0。 
...
说明 
 
bit 0-10：除远程兑换外，哨兵机器人成功兑换的允许发弹量，开局为0，
在哨兵机器人成功兑换一定允许发弹量后，该值将变为哨兵机器人成功兑
换的允许发弹量值 
 
bit 11-14：哨兵机器人成功远程兑换允许发弹量的次数，开局为0，在哨
...
大小 
说明 
兵机器人成功远程兑换允许发弹量后，该值将变为哨兵机器人成功远程兑
换允许发弹量的次数 
 
bit 15-18：哨兵机器人成功远程兑换血量的次数，开局为0，在哨兵机器
人成功远程兑换血量后，该值将变为哨兵机器人成功远程兑换血量的次数 
 
...
 
bit 0-7：哨兵进攻姿态弱化前剩余可持续时长（单位秒，下同） 
 
...
说明 
 
bit 0-1：雷达是否拥有触发双倍易伤的机会，开局为0，数值为雷达拥
有触发双倍易伤的机会，至多为2 
 
bit 2：对方是否正在被触发双倍易伤 
 
...
发送者和接收者的ID、内容数据段。机器人交互数据包的总长不超过127 个字节，减去frame_header、
cmd_id 和frame_tail 的9 个字节以及数据段头结构的6 个字节，故机器人交互数据的内容数据段最大
为112 个字节。 
每1000 毫秒，英雄、工程、步兵、空中机器人、飞镖能够接收数据的上限为3720 字节，雷达和哨兵机器
人能够接收数据的上限为5120 字节。 
由于存在多个内容ID，但整个cmd_id 上行频率最大为30Hz，请合理安排带宽。 
表 1-25 0x0301 
字节偏移量 
大小 
说明 
...
需为规则允许的多机通讯接收者 
 
若接收者为选手端，则除雷达系统外，仅可发送至发送者
对应的选手端。雷达系统可发送至任意己方选手端。ID 编
号详见附录 
x 
内容数据段 
x 最大为112 
...
8：白色 
 
bit 14-31：根据绘制的图形不同，含义不同，详见“表 1-28 
图形细节参数说明” 
图形配置2 
 
...
bit 21-31：起点/圆心y 坐标 
图形配置3 
根据绘制的图形不同，含义不同，详见“表 1-28 图形细节参
数说明” 
typedef _packed struct 
{ 
...
 
浮点数：整型数均为32 位，对于浮点数，实际显示的值为输入的值/1000，如在details_c、
details_d、details_e 对应的字节输入1234，选手端实际显示的值将为1.234。 
 
...
bit 0：哨兵机器人是否确认复活。 
 
0 表示哨兵机器人确认不复活，即使此时哨兵的复活
读条已经完成。 
 
1 表示哨兵机器人确认复活，若复活读条完成将立即
复活。 
 
...
币兑换立即复活。 
 
bit 2-12：哨兵将要兑换的发弹量值，开局为0，修改此值
后，哨兵在补血点即可兑换允许发弹量。 
此值的变化需要单调递增，否则视为不合法。 
示例：此值开局仅能为0，此后哨兵可将其从0 修改至
X，则消耗X 金币成功兑换X 允许发弹量。此后哨兵可
将其从X 修改至X+Y，以此类推。 
 
bit 13-16：哨兵远程兑换发弹量的请求次数，开局为0，
修改此值即可请求远程兑换发弹量。 
此值的变化需要单调递增且每次仅能增加1，否则视为不
合法。 
示例：此值开局仅能为0，此后哨兵可将其从0 修改至
1，则消耗金币远程兑换允许发弹量。此后哨兵可将其从
1 修改至2，以此类推。 
 
bit 17-20：哨兵远程兑换血量的请求次数，开局为0，修
改此值即可请求远程兑换血量。 
此值的变化需要单调递增且每次仅能增加1，否则视为不
合法。 
示例：此值开局仅能为0，此后哨兵可将其从0 修改至
1，则消耗金币远程兑换血量。此后哨兵可将其从1 修改
至2，以此类推。 
在哨兵发送该子命令时，服务器将按照从相对低位到相对
高位的原则依次处理这些指令，直至全部成功或不能处理
为止。 
示例：若队伍金币数为0，此时哨兵战亡，“是否确认复
活”的值为1，“是否确认兑换立即复活”的值为1，“确
认兑换的允许发弹量值”为100。（假定之前哨兵未兑换
过允许发弹量）由于此时队伍金币数不足以使哨兵兑换
立即复活，则服务器将会忽视后续指令，等待哨兵发送
的下一组指令。 
 
bit 21-23：哨兵修改当前姿态指令，1 为进攻姿态，2 为
防御姿态，3 为移动姿态，4 为强化进攻姿态，5 为强化
防御姿态，6 为强化移动姿态。默认为3；修改此值即可
改变哨兵姿态。 
...
雷达是否确认触
发双倍易伤 
开局为0，修改此值即可请求触发双倍易伤，若此时雷
达拥有触发双倍易伤的机会，则可触发。 
此值的变化需要单调递增且每次仅能增加1，否则视为
不合法。 
示例：此值开局仅能为0，此后雷达可将其从0 修改至
1，若雷达拥有触发双倍易伤的机会，则触发双倍易
伤。此后雷达可将其从1 修改至2，以此类推。 
若雷达请求双倍易伤时，双倍易伤正在生效，则第二次
双倍易伤将在第一次双倍易伤结束后生效。 
密钥更新或验证
指令 
每个字节均为ASCII 码编码的字母或数字。开局为随机
值。byte1 为指令类型，byte2-7 为密钥值。 
当byte1 值为1 时，修改此值即可更新己方加密密钥；
当byte1 值为2 时，修改此值即可将雷达破解的对方密
钥传输给服务器以验证是否正确破解。 
注意： 
 
仅开局和每次对方破解成功使得加密等级（己方干
扰波难度）提高时可以修改密钥，其余时间修改无
效。（即使不主动修改，密钥也将重置为一个新的随
机值）。 
 
当byte1 值为2 时，每次更新验证密钥后的10 秒
内，再次更新无效。 
...
命令码为0x0303，触发时发送，两次发送间隔不得低于0.5 秒。 
发送方式一： 
① 点击己方机器人头像； 
...
命令码为0x0303，触发时发送，两次发送间隔不得低于3 秒。 
发送方式： 
① （可选）按下一个键盘按键或点击对方机器人头像； 
...
处理，具体如下：选手端触发1 次发送后，服务器将以100ms 的间隔向机器人额外发送4 次，
共5 次。此后，直到下一次选手端触发发送前，服务器都将以1Hz 的频率持续定频发送最近
一次的包。触发时的连续发送和1Hz 定频发送计时相互独立。队伍需关注多次收到重复协议
内容的处理方式。 
表 1-35 命令码ID：0x0303 
字节偏移量 
大小 
...
此机器人坐标。 
对方工程机器人y 位置坐标，单位：cm 
对方3 号步兵机器人x 位置坐标，单位：cm 
对方3 号步兵机器人y 位置坐标，单位：cm 
对方4 号步兵机器人x 位置坐标，单位：cm 
对方4 号步兵机器人y 位置坐标，单位：cm 
对方6 号空中机器人x 位置坐标, 单位：cm 
对方6 号空中机器人y 位置坐标, 单位：cm 
对方哨兵机器人x 位置坐标，单位：cm 
对方哨兵机器人y 位置坐标，单位：cm 
己方英雄机器人x 位置坐标，单位：cm 
己方英雄机器人y 位置坐标，单位：cm 
己方工程机器人x 位置坐标，单位：cm 
己方工程机器人y 位置坐标，单位：cm 
己方3 号步兵机器人x 位置坐标，单位：cm 
己方3 号步兵机器人y 位置坐标，单位：cm 
己方4 号步兵机器人x 位置坐标，单位：cm 
己方4 号步兵机器人y 位置坐标，单位：cm 
己方6 号空中机器人x 位置坐标，单位：cm 
己方6 号空中机器人y 位置坐标，单位：cm 
己方哨兵机器人x 位置坐标，单位：cm 
己方哨兵机器人y 位置坐标，单位：cm 
...
备注 
路径起点x 轴坐标，单位：dm 
小地图左下角为坐标原点，水平向右为X 轴
正方向，竖直向上为Y 轴正方向。显示位置
将按照场地尺寸与小地图尺寸等比缩放，超
出边界的位置将在边界处显示 
路径起点y 轴坐标，单位：dm 
路径点x 轴增量数组，单位：dm 
增量相较于上一个点位进行计算，共49 个新
点位，X 与Y 轴增量对应组成点位 
路径点y 轴增量数组，单位：dm 
发送者ID 
需与自身ID 匹配，ID 编号详见附录 
...
需要校验发送者的ID 正确性 
接收者的ID 
需要校验接收者的ID 正确性，仅支持发送己方选手端 
字符 
以utf-16 格式编码发送，支持显示中文。编码发送时请注
意数据的大小端问题 
typedef _packed struct 
{ 
  uint16_t sender_id; 
...
字节偏移量 大小 
说明 
对方英雄机器人位置x 轴坐标，单位：cm 
对方英雄机器人位置y 轴坐标，单位：cm 
对方工程机器人位置x 轴坐标，单位：cm 
对方工程机器人位置y 轴坐标，单位：cm 
对方3 号步兵机器人位置x 轴坐标，单位：cm 
对方3 号步兵机器人位置y 轴坐标，单位：cm 
对方4 号步兵机器人位置x 轴坐标，单位：cm 
对方4 号步兵机器人位置y 轴坐标，单位：cm 
对方空中机器人位置x 轴坐标，单位：cm 
对方空中机器人位置y 轴坐标，单位：cm 
对方哨兵机器人位置x 轴坐标，单位：cm 
对方哨兵机器人位置y 轴坐标，单位：cm 
...
大小 
说明 
对方1 号英雄机器人血量，若该机器人未上场或者被罚下，则血量为0，下文
同理 
对方2 号工程机器人血量 
对方3 号步兵机器人血量 
对方4 号步兵机器人血量 
...
说明 
对方1 号英雄机器人允许发弹量 
对方3 号步兵机器人允许发弹量（含堡垒提供的储备允许发弹量，下同） 
对方4 号步兵机器人允许发弹量 
对方6 号空中机器人允许发弹量 
对方7 号哨兵机器人允许发弹量 
表 1-47 命令码ID：0x0A04 
...
 
bit 4-5：对方堡垒增益点的占领状态，0 为未被占领，1 为被对方占领，2
为被己方占领，3 为被双方占领 
...
说明 
 
bit 6-7：对方前哨站增益点的占领状态，0 为未被占领，1 为被对方占领，
2 为被己方占领 
 
bit 8：对方基地增益点的占领状态，1 为已占领 
 
bit 9：靠近对方一侧飞坡前地形跨越增益点（隧道）中心处场地交互模块
卡的状态，1 为检测到对方机器人场地交互模块 
 
bit 10：靠近对方一侧飞坡后地形跨越增益点（隧道）中心处场地交互模块
卡的状态，1 为检测到对方机器人场地交互模块 
 
bit 11：靠近己方一侧飞坡前地形跨越增益点（隧道）中心处场地交互模块
卡的状态，1 为检测到对方机器人场地交互模块 
 
bit 12：靠近己方一侧飞坡后地形跨越增益点（隧道）中心处场地交互模块
卡的状态，1 为检测到对方机器人场地交互模块 
 
bit 13：对方地形跨越增益点（高地）上部场地交互模块卡的状态，1 为检
测到对方机器人场地交互模块 
 
bit 14：对方地形跨越增益点（飞坡）后部场地交互模块卡的状态，1 为检
测到对方机器人场地交互模块 
 
bit 15：对方地形跨越增益点（公路）上部场地交互模块卡的状态，1 为检
测到对方机器人场地交互模块 
表 1-48 命令码ID：0x0A05 
字节偏移量 大小 
说明 
对方英雄机器人回血增益（百分比，值为 10 表示每秒恢复血量上限的10%，
下同） 
对方英雄机器人射击热量冷却增益具体值（直接值，值为x 表示热量冷却增加
x/s，下同） 
对方英雄机器人防御增益（百分比，值为 50 表示50%防御增益，下同） 
对方英雄机器人负防御增益（百分比，值为30 表示-30%防御增益，下同） 
对方英雄机器人攻击增益（百分比，值为50 表示50%攻击增益，下同） 
对方工程机器人回血增益 
...
对方工程机器人攻击增益 
对方3 号步兵机器人回血增益 
对方3 号步兵机器人射击热量冷却增益具体值 
对方3 号步兵机器人防御增益 
对方3 号步兵机器人负防御增益 
对方3 号步兵机器人攻击增益 
对方4 号步兵机器人回血增益 
对方4 号步兵机器人射击热量冷却增益具体值 
对方4 号步兵机器人防御增益 
对方4 号步兵机器人负防御增益 
对方4 号步兵机器人攻击增益 
对方哨兵机器人回血增益 
...
对方哨兵机器人负防御增益 
对方哨兵机器人攻击增益 
对方哨兵机器人当前姿态（1 为进攻姿态，2 为防御姿态，3 为移动姿态，4 为
强化进攻姿态，5 为强化防御姿态，6 为强化移动姿态） 
对方英雄机器人主要状态（0 为存活，1 为战亡，2 为无敌但不虚弱，3 为无敌
且虚弱，异常离线不会影响此处的状态判断，下同） 
...
大小 
说明 
每个字节均为ASCII 码编码的字母或数字。 

 
 
2 自定义客户端协议 
自定义客户端的数据流格式为Protobuf v3 （Protocol Buffers）,支持版本为3.15 以上,采用MQTT 作
为发布/订阅消息传输协议。MQTT 直接传输Protobuf 序列化的二进制流。topic 为对应的指令名。服务
器IP 地址为固定IP：192.168.12.1，端口：3333。自定义客户端IP 地址为192.168.12.2。 
通信流程如下： 
1. 
结合文档中的Protobuf 消息格式编写proto 文件，用protoc 生成不同语言的类代码 
2. 
在发布者端，将对象序列化成二进制 
3. 
通过MQTT publish 把二进制发送到对应Topic 
4. 
在订阅者端，接收消息并用Protobuf 反序列化 
示例代码，详见“附录三：自定义客户端示例通信代码”，其中MyMqttClient 的clientID 需填入所连接
机器人对应的ID 编号。 
此外，自定义客户端可通过udp 监听3334 端口获取图传码流数据。图传码流数据的帧率为60 帧，分辨
率为1920*1080，编码格式为hevc，标准h265 格式，无重传机制。 
每个udp 包的码流数据的前8 个字节固定为： 
帧编号（递增）：2 byte 
当前帧内分片序号：2 byte 
当前帧总字节数：4 byte 
此额外添加的8 个字节为大端传输。每个udp 包最大为1400 个字节，除去8 个固定字节外，大小为1392。 
2.1 指令概览 
表 2-1 指令概览 
指令名 
指令用途 
...
键盘按键位掩码：注释：数据类型虽为uint32，但实际仅需发送bit 0 到bit 15 范围的数据即可。 
对应关系： 
每个bit 对应一个按键，0 为未按下，1 为按下： 
 
bit 0：W 键 
 
bit 1：S 键 
...
2.2.2 CustomControl 
用途：最大30 字节的自定义数据。 
说明：该信息将通过图传链路以0x0311 命令字发送给机器人。 
数据编号 
数据类型 
数据用途 
bytes 
...
数据用途 
uint32 
当局胜者。0 为平局，1 为红方胜利，2 为蓝方胜利。仅在“当前阶
段”为“比赛结算中”时该值有意义，其余时间发送255 
uint32 
比赛结束原因。仅在“当前阶段”为“比赛结算中”时该值有意义，
其余时间发送255 
current_stage 枚举值： 
 
0：未开始比赛 
...
对方前哨站状态 
repeated uint32 
所有机器人血量（按照己方1 号，2 号，3 号，4 号，7 号，
对方1 号，2 号，3 号，4 号，7 号的顺序发送） 
repeated int32 
己方机器人剩余累计发弹量 
uint32 
己方累计总伤害 
...
event_id 枚举值： 
 
1：击杀事件（参数1 为被击杀者，参数2 为击杀者，如“1,101”，代表蓝方英雄击毁红方英雄） 
 
...
 
9：飞镖命中（第一个参数为命中方，1 为红方命中，2 为蓝方命中 第二个参数值为命中目标，1 为击
中前哨站，2 为击中基地固定目标，3 为击中基地随机固定目标，4 为击中基地随机移动目标，5 为击
中基地末端移动目标） 
 
10：对方飞镖闸门开启（无参数） 
 
...
 
15：装配结果事件(0 成功，1 拔出，2 超时，3 离开装配区过久，4 工程战亡，5 四级难度未满足完成
协作时限，6 主动退出，7 完成装配但结算时未检测到有能量单元，8 缓冲期到期，装配流程强制结束) 
message Event { 
...
数据用途 
uint32 
电源管理模块状态（0=离线，1=在线，2=因安装不规范被视为离线） 
...
数据用途 
uint32 
RFID 模块状态（0=离线，1=在线，2=因安装不规范被视为离线） 
uint32 
灯条模块状态（0=离线，1=在线，2=因安装不规范被视为离线） 
uint32 
17mm 发射机构状态（0=离线，1=在线，2=因安装不规范被视为离线） 
uint32 
42mm 发射机构状态（0=离线，1=在线，2=因安装不规范被视为离线） 
uint32 
定位模块状态（0=离线，1=在线，2=因安装不规范被视为离线） 
uint32 
装甲模块状态（0=离线，1=在线，2=因安装不规范被视为离线） 
uint32 
图传模块状态（0=离线，1=在线，2=因安装不规范被视为离线） 
uint32 
电容模块状态（0=离线，1=在线，2=因安装不规范被视为离线） 
uint32 
主控状态（0=离线，1=在线，2=因安装不规范被视为离线） 
uint32 
激光检测模块状态（0=离线，1=在线，2=因安装不规范被视为离线） 
...
本机器人测速模块的朝向，单位：度，正北为0 度 
uint32 
接受到的坐标归属机器人，6/106 为空中机器人，7/107 为哨兵
机器人（仅云台手客户端生效） 
...
Buff 类型 
int32 
Buff 增益值（当buff 类型枚举值为1，2，4，5 时，此值
为百分比，如Buff 类型为防御增益，增益值为50 表示
50%防御增益；当buff 类型为3 时，此值为直接值，值为
x 表示热量冷却增加x/s） 
uint32 
Buff 最大剩余时间 
uint32 
...
起始点Y 坐标（分米） 
repeated int32 
相对起始点X 增量数组（-128~+127，长度49） 
repeated int32 
相对起始点Y 增量数组（-128~+127，长度49） 
uint32 
发送者ID 
...
bytes 
目标机器人ID 列表（固定7 字节，如发送给己方英雄和工程机
器人，红方需要填入1，2；蓝方需要填入101，102，后续填0） 
uint32 
...
数据用途 
uint32 
自定义图标ASCII 码（仅在标记类型为4：自定义时有效。仅允
许发送官方选手端可接受的字母，范围为从（含）“C”到
“L”，“N”，“O”，“Q”到“Z”） 
uint32 
标记模式（1=地图，2=对方机器人） 
float 
...
数据类型 
repeated uint32 
雷达发送的所有机器人位置信息及特殊标识情况，每次触发发送时均按对方1 号，2 号，3
号，4 号，6 号，7 号；己方1 号，2 号，3 号，4 号，6 号，7 号的排列顺序发送以下结构
的数据包： 
数据编号 
数据类型 
数据用途 
...
当前可选择的最高装配难度等级 
uint32 
科技核心基础状态，1 为初始位置，2 为运动中，3 为已到达对应位姿 
...
数据用途 
uint32 
能量单元放入状态，0 为未放入，1 为已经放入 
uint32 
能量单元平移状态，0 为未平移，1 为已经平移 
uint32 
能量单元旋转状态，0 为未旋转，1 为已经旋转 
uint32 
对方科技核心状态（仅己方四级装配可用时有效，0 为对方没有正在
装配，1 为对方正在装配非四级难度，2 为对方正在装配四级难度） 
uint32 
...
cmd_type 枚举值： 
 
1：兑换17mm 发弹量（仅剩余金币足以兑换时有效；该命令类型对应的参数值必须为10 的倍数） 
 
2：兑换42mm 发弹量（仅剩余金币足以兑换时有效） 
 
...
数据用途 
uint32 
目标ID（1 为前哨站，2 为基地固定目标，3 为基地随机固
定目标，4 为基地随机移动目标，5 为基地末端移动目标） 
bool 
闸门开关 
bool 
是否确认发射（默认为0，1 为确认发射） 
...
0x0102：红方工程机器人选手端 
 
0x0103/0x0104/0x0105：红方步兵机器人选手端（与机器人ID 3~5 对应） 
 
0x0106：红方空中机器人选手端 
 
0x016A：蓝方空中机器人选手端 
...
0x0166：蓝方工程机器人选手端 
 
0x0167/0x0168/0x0169：蓝方步兵机器人选手端（与机器人ID 3~5 对应） 
 
...
float target_position_x; 
float target_position_y; 
float target_position_yaw; 
} ext_robot_command_t 
机器人ID：1，英雄（红）；2，工程（红）；3/4/5，步兵（红）；6，空中（红）；7，哨兵（红）；8，基地
（红），9，前哨站（红）11，英雄（蓝）；12，工程（蓝）；13/14/15，步兵（蓝）；16，空中（蓝）；17，
哨兵（蓝）；18 基地（蓝），19 前哨站（蓝） 
