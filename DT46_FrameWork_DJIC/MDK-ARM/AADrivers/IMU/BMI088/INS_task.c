...
float IMU_Yaw;
float IMU_Pitch;
float IMU_DegAngle[3];

#define MY_PI_F 3.14159265358979323846f

float radian_to_degree(float radian) {
    return radian * (180.0f / MY_PI_F);  // 使用float常量
}

void GetIMU_Value(void)
{
	IMU_Yaw = radian_to_degree(INS_angle[0]);
	IMU_Pitch = radian_to_degree(INS_angle[1]);
	IMU_DegAngle[0] = radian_to_degree(INS_angle[0]);
