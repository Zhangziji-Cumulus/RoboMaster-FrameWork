...
void Gimbal_Init(void)
{
    ...
    //自瞄融合系数初始化
    Gimbal_Instance.K_Aim = 0.0f;

    //纯手动目标角度初始化（停止后会通过SetSafe_Vel锁到当前角度）
    Gimbal_Instance.Calc.Yaw.Man_T_Angle = 0.0f;
    Gimbal_Instance.Calc.Pitch.Man_T_Angle = 0.0f;
...
static void Gimbal_Update_Target(void)
{
    ...
    Gimbal_Instance.Calc.Yaw.Man_T_Angle -= ManualYaw;
    Gimbal_Instance.Calc.Pitch.Man_T_Angle -= ManualPitch;

    //手动目标限幅
    Gimbal_Instance.Calc.Yaw.Man_T_Angle = MyMath_Limit_Float(
                                            Gimbal_Instance.Calc.Yaw.Man_T_Angle,
                                            -180.00f,180.00f,1);
    Gimbal_Instance.Calc.Pitch.Man_T_Angle = MyMath_Limit_Float(
                                            Gimbal_Instance.Calc.Pitch.Man_T_Angle,
                                            -GIMBAL_PITCH_MAX_DEP,GIMBAL_PITCH_MAX_ELE,0);

    /* ===== 2. K_Aim 渐变 ===== */
    bool aim_active = (Gimbal_Instance.CMD.Auto.Aim == AUTOAIM_ON) 
                   && Gimbal_Instance.Auto.Aim.IsOnline;

    if(aim_active)
    {
        if(Gimbal_Instance.K_Aim < AUTOAIM_K_AIM_MAX)
            Gimbal_Instance.K_Aim += AUTOAIM_K_AIM_STEP;
    }
    else
    {
        if(Gimbal_Instance.K_Aim > 0.0f)
            Gimbal_Instance.K_Aim -= AUTOAIM_K_AIM_STEP;
    }

    if(Gimbal_Instance.K_Aim > AUTOAIM_K_AIM_MAX) Gimbal_Instance.K_Aim = AUTOAIM_K_AIM_MAX;
    if(Gimbal_Instance.K_Aim < 0.0f) Gimbal_Instance.K_Aim = 0.0f;

    /* ===== 3. 最终目标 = 手动位置 + 自瞄偏移 ===== */
    Gimbal_Instance.Calc.Yaw.T_Angle = Gimbal_Instance.Calc.Yaw.Man_T_Angle
                                     + Gimbal_Instance.K_Aim * Gimbal_Instance.Auto.Aim.Yaw;

    Gimbal_Instance.Calc.Pitch.T_Angle = Gimbal_Instance.Calc.Pitch.Man_T_Angle
                                       + Gimbal_Instance.K_Aim * Gimbal_Instance.Auto.Aim.Pitch;
...
static void Gimbal_SetSafe_Vel(void)
{
    ...
    Gimbal_Instance.Calc.Yaw.Man_T_Angle = Gimbal_Instance.Calc.Yaw.C_Angle;
    Gimbal_Instance.Calc.Pitch.T_Angle = Gimbal_Instance.Calc.Pitch.C_Angle;
    Gimbal_Instance.Calc.Pitch.Man_T_Angle = Gimbal_Instance.Calc.Pitch.C_Angle;
    Gimbal_Instance.K_Aim = 0.0f;
}
